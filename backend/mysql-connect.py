...
def info_parsing(directories, num, cursor):
    """
    Parse every object from the .json files
    :param cursor:
    :param directories:
    :param num:
    :return:
    """
    try:
        pokeapi_obj = loadfiles(directories[0], num)
        pokedex_api_obj = loadfiles(directories[1], num)
    except Exception as e:
        raise e
    # Main =====================================================
    id_num = pokeapi_obj['id']  # Pokemon and index number
    name = pokeapi_obj['name'].capitalize()
